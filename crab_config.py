...
year = '2018'            # Defina o ano aqui
data_type = 'MC'       # Use 'DATA' para dados reais ou 'MC' para Monte Carlo

config = config()

config.section_("General")
config.General.workArea = '27JanMETReview_crab_EXOMONOPOLE_{data_type}_{year}/{era}'.format(data_type=data_type, year=year, era='{era}')
...
if data_type == 'DATA':
    config.JobType.psetName = 'ntuple_forData_cfg_METFilter.py'
elif data_type == 'MC':
    if year == '2016':
        ...
        config.JobType.psetName = 'ntuple_mc_2018_cfg_METFilter_Review.py'
    else:
...
for dataset in datasets:
    ...
    crab_config.General.requestName = '27Jan_EXOMETReview_{data_type}_{year}_{era}'.format(data_type=data_type, year=year, era=era)
    ...
    crab_config.Data.outLFNDirBase = '/store/user/tmenezes/JanEXOMETReview_MagneticMonopole_{data_type}_{year}/{era}/'.format(data_type=data_type, year=year, era=era)
